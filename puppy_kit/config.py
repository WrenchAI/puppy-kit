...
def get_config_path() -> str:
    ...
    return os.path.join(os.path.expanduser("~"), ".puppy-kit", "config.json")
...
def _load_profile_data(profile: str | None = None) -> dict | None:
    ...
    Args:
        profile: Profile name to load. If None, uses PUPPY_KIT_PROFILE env var
    ...
    if profile is None:
        profile = os.environ.get("PUPPY_KIT_PROFILE", data.get("active_profile", ""))
...
def load_config(profile: str | None = None) -> DatadogConfig:
    ...
    Args:
        profile: Optional profile name to use. If specified, loads from
                 the named profile in ~/.puppy-kit/config.json.
    """
    # Try loading from profile (handles CLI flag, PUPPY_KIT_PROFILE, active_profile)
...
def _print_config_help() -> None:
    ...
    console.print("\n[dim]Or configure a profile: puppy config init[/dim]")
