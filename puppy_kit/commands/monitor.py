...
def mute_monitor(monitor_id, scope, duration, from_stdin):
    ...
    Examples:
        puppy_kit monitor mute 12345
        puppy_kit monitor mute 12345 --duration 3600
        puppy_kit monitor list --state Alert --format json | puppy_kit monitor mute --from-stdin
    """
    import time
...
def create_monitor_cmd(
    ...
    Examples:
        puppy_kit monitor create --type "metric alert" --query "..." --name "CPU Alert"
        puppy_kit monitor create -f monitor.json
        echo '{"type":"metric alert","query":"...","name":"CPU"}' | puppy_kit monitor create --from-stdin
    """
    from datadog_api_client.v1.model.monitor import Monitor
...
def update_monitor_cmd(
    ...
    Examples:
        puppy_kit monitor update 12345 --name "New Name"
        puppy_kit monitor update 12345 -f update.json
        echo '{"name":"New Name"}' | puppy_kit monitor update 12345 --from-stdin
    """
    from datadog_api_client.v1.model.monitor_update_request import MonitorUpdateRequest
