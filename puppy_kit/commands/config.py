...
def get_config_dir() -> str:
    """Return the path to the config directory (~/.puppy-kit)."""
    return os.path.join(os.path.expanduser("~"), ".puppy-kit")


def get_config_path() -> str:
    """Return the path to the config file (~/.puppy-kit/config.json)."""
...
def set_profile(name, api_key, app_key, site, ops_profile):
    ...
    Example:
        puppy config set-profile prod --api-key xxx --app-key yyy --site us
    """
    site = expand_site(site)
...
def use_profile(name):
    ...
    if not data:
        console.print("[red]No config file found. Run 'puppy config init' first.[/red]")
        sys.exit(1)
...
def list_profiles():
    ...
    if not data or not data.get("profiles"):
        console.print("[yellow]No profiles configured. Run 'puppy config init' to set up.[/yellow]")
        return
...
def get_value(key):
    ...
    if active not in profiles:
        console.print("[red]No active profile set. Run 'puppy config use-profile <name>'.[/red]")
        sys.exit(1)
