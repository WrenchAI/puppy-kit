...
def apply_cmd(file_path, from_stdin, dry_run, recursive):
    ...
    Examples:
        puppy_kit apply -f monitor.json
        puppy_kit apply -f dashboards/ --recursive
        puppy_kit apply -f slo.json --dry-run
        echo '{"type":"metric alert","query":"...","name":"CPU"}' | puppy_kit apply --from-stdin
    """
    if from_stdin:
...
def diff_cmd(file_path):
    ...
    Examples:
        puppy_kit diff -f monitor.json
        puppy_kit diff -f dashboard.json
    """
    try:
