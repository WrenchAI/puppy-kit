...
def create_dashboard_cmd(title, layout_type, description, file_data, from_stdin, fmt):
    ...
    From file (primary workflow):
        puppy_kit dashboard create -f dashboard.json

    From stdin (piping):
        puppy_kit dashboard export abc-123 | puppy_kit dashboard create --from-stdin --title "Copy"

    With inline flags (metadata only, no widgets):
        puppy_kit dashboard create --title "My Dashboard" --layout-type ordered
    """
    if from_stdin:
...
def update_dashboard_cmd(dashboard_id, file_data, fmt):
    ...
    Usage:
        puppy_kit dashboard update <dashboard_id> -f dashboard.json
    """
    if not file_data:
